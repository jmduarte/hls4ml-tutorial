...
typedef struct {
    ...
    XJet_tagger_axi_uio_map maps[ MAX_UIO_MAPS ];
} XJet_tagger_axi_uio_info;

/***************** Variable Definitions **************************************/
static XJet_tagger_axi_uio_info uio_info;
...
static int uio_info_read_name(XJet_tagger_axi_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XJet_tagger_axi_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XJet_tagger_axi_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XJet_tagger_axi_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XJet_tagger_axi_Initialize(XJet_tagger_axi *InstancePtr, const char* InstanceName) {
	XJet_tagger_axi_uio_info *InfoPtr = &uio_info;
